"""
THE TAMERS USERBOT v2.0 - BLOOD EDITION (ORIGINAL)
...
async def cmd_status(client, message):
    ...
    async for d in client.get_dialogs():
        if d.chat.type == ChatType.PRIVATE and d.chat.id > 0:
            ...
            grp += 1
        elif d.chat.type == ChatType.CHANNEL:
            ch += 1
    await message.reply(
        f"🩸 **STATUS DARAH**\n"
        f"┃ tamer    : {me.first_name}\n"
...
async def blood_reply_handler(client, message):
    ...
    if AFK_MODE and message.chat.type == ChatType.PRIVATE:
        ...
        if cnt >= 5:
            ...
            return
        if cnt >= 3 and not AFK_PENDING[uid].get("warned", False):
...
async def main():
    ...
    print("🩸"*30)
    print("🩸 THE TAMERS BLOOD EDITION (ORIGINAL)")
